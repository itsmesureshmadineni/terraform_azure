...
resource "azurerm_network_interface_backend_address_pool_association" "web_lb_backendpool_asso" {
  ...
  network_interface_id = azurerm_network_interface.web_linuxvn_ni[count.index].id
  ip_configuration_name = azurerm_network_interface.web_linuxvn_ni[count.index].ip_configuration[0].name
